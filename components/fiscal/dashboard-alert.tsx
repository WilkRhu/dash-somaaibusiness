...
export function DashboardAlert({ type, title, message, action }: DashboardAlertProps) {
  const typeClasses = {
  ...
  const getIcon = () => {
    switch (type) {
      case 'error':
        return (
          <svg className={`w-5 h-5 flex-shrink-0 ${iconClasses[type]}`} fill="currentColor" viewBox="0 0 20 20">
