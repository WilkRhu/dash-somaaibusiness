...
// Cores de sombra por status
const shadowColors: Record<DeliveryStatus, { rgb: string; ring: string }> = {
  [DeliveryStatus.PENDING]: { rgb: 'rgba(234, 179, 8, 0.5)', ring: 'ring-yellow-400' },
  [DeliveryStatus.CONFIRMED]: { rgb: 'rgba(59, 130, 246, 0.5)', ring: 'ring-blue-400' },
  [DeliveryStatus.PREPARING]: { rgb: 'rgba(147, 51, 234, 0.5)', ring: 'ring-purple-400' },
  [DeliveryStatus.READY_FOR_DELIVERY]: { rgb: 'rgba(249, 115, 22, 0.5)', ring: 'ring-orange-400' },
  [DeliveryStatus.OUT_FOR_DELIVERY]: { rgb: 'rgba(99, 102, 241, 0.5)', ring: 'ring-indigo-400' },
  [DeliveryStatus.DELIVERED]: { rgb: 'rgba(34, 197, 94, 0.5)', ring: 'ring-green-400' },
  [DeliveryStatus.CANCELLED]: { rgb: 'rgba(239, 68, 68, 0.5)', ring: 'ring-red-400' },
  [DeliveryStatus.FAILED]: { rgb: 'rgba(107, 114, 128, 0.5)', ring: 'ring-gray-400' },
};

// Verificar se o pedido está ativo (não entregue nem cancelado)
const isOrderActive = (status: DeliveryStatus) => {
...
export function DeliveryOrderCard({ order, onViewDetails, onUpdateStatus }: DeliveryOrderCardProps) {
  ...
  const shadowColor = shadowColors[order.status];
  const shadowRgb = shadowColor.rgb;
  const shadowRgbBright = shadowColor.rgb.replace('0.5', '0.8');

  return (
    <>
      <style>{`
        @keyframes pulse-shadow-${order.id} {
          0%, 100% {
            box-shadow: 0 0 20px ${shadowRgb};
          }
          50% {
            box-shadow: 0 0 30px ${shadowRgbBright};
          }
        }
      `}</style>
      <div
        className={`bg-white rounded-lg shadow p-4 hover:shadow-md transition-all ${
          isActive ? `ring-2 ${shadowColor.ring}` : ''
        }`}
        style={
          isActive
            ? {
                boxShadow: `0 0 20px ${shadowRgb}`,
                animation: `pulse-shadow-${order.id} 2s cubic-bezier(0.4, 0, 0.6, 1) infinite`,
              }
            : {}
        }
      >
