...
function GoogleLoginContent({ 
  ...
  const handleSuccess = async (credentialResponse: any) => {
    console.log('🔐 onSuccess chamado com:', JSON.stringify(credentialResponse));
    
    if (!credentialResponse?.credential) {
      console.error('❌ credential não encontrado:', credentialResponse);
      alert('Erro ao obter credencial do Google. Tente novamente.');
      onError?.();
      return;
    }
    ...
    try {
      ...
      onSuccess?.();
      router.push(isSuperAdmin ? '/admin' : '/home');
    } catch (error: any) {
