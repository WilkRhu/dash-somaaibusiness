...
export class GetnetTerminalProvider implements IPaymentTerminal {
  ...
  async setup(config: PaymentTerminalConfig): Promise<void> {
    ...
    console.log('Getnet terminal configurado com sucesso');
  }

  async processPayment(request: ProcessPaymentRequest): Promise<ProcessPaymentResponse> {
    if (!this.isConnected) {
      throw new Error('Terminal Getnet não está conectado');
    }

    // TODO: Implementar integração com Getnet API
  ...
  async refund(transactionId: string, request: RefundRequest): Promise<RefundResponse> {
    ...
    // TODO: Implementar integração com Getnet API para reembolso
  ...
  async disconnect(): Promise<void> {
    ...
    console.log('Getnet terminal desconectado');
  }
}
