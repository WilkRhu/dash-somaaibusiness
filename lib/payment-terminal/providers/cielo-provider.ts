...
export class CieloTerminalProvider implements IPaymentTerminal {
  ...
  async setup(config: PaymentTerminalConfig): Promise<void> {
    ...
    console.log('Cielo terminal configurado com sucesso');
  }

  async processPayment(request: ProcessPaymentRequest): Promise<ProcessPaymentResponse> {
    if (!this.isConnected) {
      throw new Error('Terminal Cielo não está conectado');
    }

    // TODO: Implementar integração com Cielo API
  ...
  async refund(transactionId: string, request: RefundRequest): Promise<RefundResponse> {
    ...
    // TODO: Implementar integração com Cielo API para reembolso
  ...
  async disconnect(): Promise<void> {
    ...
    console.log('Cielo terminal desconectado');
  }
}
